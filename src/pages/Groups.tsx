...
import {
  ...
  Loader2, ChevronLeft, X, Search, Globe, MessageCircle,
} from "lucide-react";
...
export default function Groups() {
  ...
  const [activeTab, setActiveTab] = useState<"goals" | "notes" | "members">("notes");
  ...
  const fetchAll = useCallback(async () => {
    ...
    // Fetch members, goals, notes for all groups
    ...
    if (groupIds.length > 0) {
      ...
      (mems || []).forEach((m: any) => memMap[m.group_id]?.push(m));
    ...
    // Fetch friends for adding to groups
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-xs md:text-sm mt-1">
            {groups.length} groups · Collaborate with friends
          </p>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {gMembers.length} members · {completedGoals}/{gGoals.length} goals
                      </p>
                    </div>
                  </div>
                </button>
        ...
        {selectedGroup ? (
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="flex gap-1 bg-secondary rounded-lg p-1">
              {(["notes", "goals", "members"] as const).map((tab) => (
                <button
                  key={tab}
                  ...
                  <label className="cursor-pointer">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (file) handleFileUpload(selectedGroup.id, file);
                        e.target.value = "";
                      }}
                    />
                ...
                <div className="space-y-2">
                  {groupNotes.length === 0 ? (
                    ...
                    groupNotes.map((note) => {
                      ...
                      return (
                        <div key={note.id} className="p-4 rounded-xl border border-border bg-card/50 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Select value={addMemberId} onValueChange={setAddMemberId}>
                      <SelectTrigger className="flex-1">
                        <SelectValue placeholder="Add a friend to this group..." />
                      </SelectTrigger>
                      <SelectContent>
                        {friends
                          .filter((f) => !groupMembers.some((m) => m.user_id === f.id))
                          .map((f) => (
