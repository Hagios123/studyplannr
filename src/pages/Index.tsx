...
import {
  CalendarDays, CheckCircle2, Clock, TrendingUp, BookOpen, Sparkles,
  ArrowRight, LogOut, Flame, Lightbulb, Download, Target,
  MessageSquareText, Trophy, GraduationCap, Brain,
} from "lucide-react";
...
const TIPS = [
  ...
  "Try the AI Course Generator — paste any topic and get a full study course instantly.",
];
...
function StatCard({ icon: Icon, label, value, sublabel, variant = "default" }: {
  ...
  const iconVariants = { default: "text-muted-foreground", primary: "text-primary", accent: "text-accent", success: "text-success" };
...
function QuickActionCard({ icon: Icon, title, description, to, variant = "default" }: {
  icon: any; title: string; description: string; to: string;
...
const Index = () => {
  const { tasks, sessions, updateTaskStatus, exportSchedule, flashcards } = useStudyStore();
  ...
  const totalMinutesToday = sessions.filter((s) => s.date === today).reduce((acc, s) => acc + s.duration, 0)
    + todaysTasks.filter((t) => t.status === "completed").reduce((a, t) => a + t.duration, 0);
  ...
  const confirmComplete = () => {
    if (noteDialogTask) {
      ...
      if (newCompleted === todaysTasks.length) {
        setConfettiTrigger((c) => c + 1);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `study-schedule-${today}.txt`;
    a.click();
    URL.revokeObjectURL(url);
    toast({ title: "Schedule exported!" });
  };

  const unmasteredCards = flashcards.filter((f) => !f.mastered).length;
  ...
  return (
    <div className="space-y-6 md:space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm md:text-base mt-1">
            {pendingToday > 0
              ...
              : "No tasks scheduled. Generate a course or head to the Planner."}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <StatCard icon={Clock} label="Study Time" value={`${Math.floor(totalMinutesToday / 60)}h ${totalMinutesToday % 60}m`} sublabel="today" variant="default" />
        <StatCard icon={TrendingUp} label="Streak" value={`${streak} day${streak !== 1 ? "s" : ""}`} sublabel={streak >= 7 ? "🔥 On fire!" : "Keep going!"} variant="accent" />
              ...
              >
                ...
                {task.status === "pending" && (
                  <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity text-xs"
                    onClick={() => updateTaskStatus(task.id, "skipped")}>Skip</Button>
                )}
              </div>
            ))}
          </div>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" size="sm" onClick={() => { setCompletionNote(""); confirmComplete(); }}>Skip Note</Button>
